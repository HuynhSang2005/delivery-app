...
How bd-issue-tracking integrates with TodoWrite, plan-work, and other skills for optimal workflow.

## Contents

- [TodoWrite Integration](#todowrite-integration) - Temporal layering pattern
- [plan-work Integration](#plan-work-integration) - Detailed implementation plans
...
### Using the plan-work Skill

When a design field needs a detailed breakdown, reference the **plan-work** skill:
...
# Then use plan-work skill to create a detailed plan (feature-plan mode)
# The skill creates: docs/plans/YYYY-MM-DD-<feature-name>.md
```

**Detailed plan structure** (from plan-work):
...
2. **Detailed plan** (plan-work): Step-by-step execution guide
3. **TodoWrite**: Current task within the plan

**During planning phase:**
1. Create bd issue with high-level design
2. If complex: Use plan-work skill to create detailed plan
...
- **Very complex feature**: bd issue + plan-work + TodoWrite
...
**Tools used**: bd-issue-tracking + plan-work skill
...
2. Use plan-work (feature-plan mode) to structure research and writing execution
...
| Detailed implementation steps | plan-work | Stepwise implementation and verification checklist |
| Research document structure | plan-work | Structured planning framework for complex document work |
...
└─ No → Use bd
   ...
   └─ Complex feature → bd issue + plan-work + TodoWrite
...
├─ Research/writing → plan-work (feature-plan mode)
├─ Complex debugging → systematic-debugging
├─ Detailed implementation → plan-work
...
- Put detailed implementation steps in bd notes (use plan-work)
- Update bd after every TodoWrite task (update at milestones)
- Use plan-work for exploratory work (defeats the purpose)

**Do**:
- Update bd when changing tools or reaching milestones
- Use TodoWrite as "working copy" of bd's NEXT section
- Link between tools (bd design field → plan-work output file path)
...
- **plan-work**: Days to weeks (detailed breakdown)
